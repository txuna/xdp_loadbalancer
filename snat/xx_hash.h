...
static unsigned int rotl (unsigned int x, int r) {
    return ((x << r) | (x >> (32 - r)));
}
// Normal stripe processing routine.
static unsigned int round_xxhash(unsigned int acc, const unsigned int input) {
    return rotl(acc + (input * PRIME2), 13) * PRIME1;
}

static unsigned int avalanche_step (const unsigned int h, const int rshift, const unsigned int prime) {
    return (h ^ (h >> rshift)) * prime;
}
// Mixes all bits to finalize the hash.
static unsigned int avalanche (const unsigned int h) {
    return avalanche_step(avalanche_step(avalanche_step(h, 15, PRIME2), 13, PRIME3), 16, 1);
}

static unsigned int endian32 (const char *v) {
...
static unsigned int fetch32 (const char *p, const unsigned int v) {
    return round_xxhash(v, endian32(p));
}

// Processes the last 0-15 bytes of p.
static unsigned int finalize (const unsigned int h, const char *p, unsigned int len) {
    return
...
static unsigned int h16bytes_4 (const char *p, unsigned int len, const unsigned int v1, const unsigned int v2, const unsigned int v3, const unsigned int v4) {
    return
...
static unsigned int h16bytes_3 (const char *p, unsigned int len, const unsigned int seed) {
    return h16bytes_4(p, len, seed + PRIME1 + PRIME2, seed + PRIME2, seed, seed - PRIME1);
}

static unsigned int xxhash32 (const char *input, unsigned int len, unsigned int seed) {
